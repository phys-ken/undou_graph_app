...
v-t を直線の折れ線（区分的に一定の加速度）以外の形、例えばガウス関数のような
滑らかな曲線で描きたいという要望がある。現在の頂点モデル（`MotionGraph.points` +
線形補間）はこれを表現できないため、対応するには曲線種別や追加パラメータを持つ
新しいモデル／エディタUXの設計が必要になる。優先度は低く、キリの良いタイミングで
再検討する。
