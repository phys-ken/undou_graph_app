...
- **原点 O は衝突検知つき**: 既定は慣習どおり軸交点の左下だが、yMin < 0 で
  軸直下の負の目盛り数値とボックスが交差する場合のみ、目盛り数値列の
  さらに左へ退避する（目盛り数値は隠さない。nami 波アプリにも同じ重なりが
  あり、こちら側で先に修正した — nami へ移植する場合は drawAxes の
  原点ブロックを参照）。
...
- プリセット定義の単一情報源は **js/styles.js の `DISPLAY_PRESETS` /
  ...
  の両方がこれを参照する。
- REST API（schema v1.1）：トップレベル `fontSize` / `displayPreset` /
